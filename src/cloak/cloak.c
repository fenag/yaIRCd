...
/** @file
   ...
   Credits for this algorithm go to UnrealIRCd's team. The algorithm is taken from the cloak module (src/modules/cloak.c
      inside Unreal source).
   The algorithm uses MD5 to compute a hash of the user's hostname / IP address, and uses 3 additional keys as salt.
      These keys should remain secret, since they are used as salt for the MD5 hash, that is, they're usedto create some
      more confusion around the value being hashed, making it harder for an attacker to brute force a cloaked host by
      iterating over every possible IP. Choosing strong, long keys with random entries should makeit nearly impossible
   ...
   The resulting hash value depends on whether a user's IP was successfully reverse looked up. If we have a valid
      ...
      `downsample(md5(md5(KEY1:host:KEY2)+KEY3))` and prepending the net prefix, where `+` means string concatenation
   ...
   We then concatenate this value with that part of the hostname that is common to other users on the same ISP, that is,
   ...
   For example, if we have the host `bl-11-252-66-85.dsl.telepac.pt`, the cloaked host will be
      `downsample(md5(md5(KEY1:bl-11-252-66-85.dsl.telepac.pt:KEY2)+KEY3)).dsl.telepac.pt`. The downsampled entry should
      ...
      assuming the next prefix is "ex", in our example, the final result would be
      `ex-downsample(md5(md5(KEY1:bl-11-252-66-85.dsl.telepac.pt:KEY2)+KEY3)).dsl.telepac.pt`. Because the result of
   ...
   The common part of the host is assumed to be everything after the first dot, which in this case is `.dsl.telepac.pt`.
      ...
      don't have reverse lookup are stillable to join.
   ...
   If it's an IPv4 address, then it matches the general form `A.B.C.D`. We define 3 values, `alpha`, `beta` and `gama`,
      such that `alpha = downsample(md5(md5("KEY2:A.B.C.D:KEY3")+"KEY1"))`, `beta =
      downsample(md5(md5("KEY3:A.B.C:KEY1")+"KEY2"))`, and `gamma = downsample(md5(md5("KEY1:A.B:KEY2")+"KEY3"))`. Thus,
      `alpha` is unique for `A.B.C.D`, `beta` is unique for `A.B.C.`, and `gamma`is unique for `A.B.`. The resulting
   ...
   Special care must be taken, since IPv6 addresses do not necessarily hold every field `a-h`. For example, `localhost`
      can be written using `::1`. This still needs some discussion, but a possible solution is toexpand every IPv6 into
...
/** Packs an MD5 hash consisting of `MD5_DIGEST_LENGTH` bytes into a singe integer.
   To do so, we check how many integers we would need to hold a hash of `MD5_DIGEST_LENGTH` bytes, which is given by
      `MD5_DIGEST_LENGTH/sizeof(unsigned int)`. We then break an integer into chunks of `(sizeof(unsigned
      int)CHAR_BIT)/(MD5_DIGEST_LENGTH/sizeof(unsigned int))`, that is,if an integer holds `N` bits, and `M` integers
      would be needed to store an MD5 hash, then we will just divide our `N` bits by `M`, so that we get equal slots
      that holda piece of the hash information. In other words, it's like we packed `(MD5_DIGEST_LENGTH/sizeof(unsigned
      int))` into a single integer.
   ...
   As we move forward in the algorithm, a mask is kept to shif the result of the current iteration's XOR into the next
      byte inside an integer. Bytes are filled from most significant toleast significant.
   It might be good to run this through a practical example and see the actual numbers. Typically, `CHAR_BIT` is 8 bits,
      integers are 32 bits, and MD5 hash values are 128-bit. Thus, to storea hash of 128-bit, we would beed `128/32 = 4`
      integers. Thus, we grab the 32 bits that compose an integer, and evenly divide them by `4`, yielding `32/4 = 8`,
      that is, 8 bits will storethe hash of 4 bytes (because the size of an integer is 4 bytes). As a consequence, each
      iteration will pick the next 4 bytes from the hash, XOR them together, and store them in the current byteposition
      from our integer.
