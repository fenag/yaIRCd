...
/* We don't want to offer command processing functions as an interface to the rest of the code, hence, we make forward
  declarations in here, not in the header file. We do so to keep cmds_unregistered and cmds_registered arrays in the top
...
/** This array holds the commands for registered connections. Each entry is an instance of `struct cmd_func`, thus, this
   ...
   this array.
 */
static struct cmd_func cmds_registered[] = {
 ...
 */
static int add_commands(struct trie_t *trie, struct cmd_func *array, size_t array_size)
{
	size_t i;
