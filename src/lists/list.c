...
struct yaircd_list {
	...
	void (*free_func)(void *); /**<Pointer to a function that knows how to free a generic data type stored in this list by the code using this module. */
};

/** This is what we associate to each word stored. Each word is denoted a node; the trie allows association of a generic data type with a word.
	Every word is associated to an instance of this structure, so that we can have more parallelism with functions like `list_find_and_execute()`.
	This is possible because each node stored holds its own mutex, thus, functions that can't possibly add or remove an element from the list are free
	to execute concurrently with other threads accessing different elements from the list. A naive implementation would use a single mutex for the whole list,
	this would quickly become a bottleneck because `list_find_and_execute()` would have to perform a global lock to execute an action in a single element, while other threads wanting to mess with
	completely different nodes would have to wait anyway.
*/
struct yaircd_node {
...
/** Wrapper structure to hold arguments to pass to `free_yaircd_node()`. See `destroy_word_list()` and `list_delete()` for further information. */
struct destroy_args_wrapper {
	int free_data; /**<Did the original caller wanted to free data? This will either be `LIST_FREE_NODE_DATA` or `LIST_NO_FREE_NODE_DATA`. */
...
/** Function that knows how to delete a trie's node.
	...
	@param arg This will always be of type `struct destroy_args_wrapper *`, and it holds important information coming from the code above, indicating if the generic data stored in a node by the upper caller
			   is to be freed.
*/
...
/** Initializes a new, empty list, with the necessary structures to control concurrent thread access.
	@param free_function Each word can be associated to a generic pointer hereby denoted `data`. This function will be called to free a node's `data` when it is removed from the list.
						 It can be NULL if nothing shall be done when deleting a node. See `destroy_word_list()` and `list_delete()` for further info.
...
/** Finds a given word, and returns the data associated with this word, if a match was found.
	This function does not lock the global list mutex, it should only be called from within a function that is holding a lock.
...
/** Finds and performs an action on a word's data atomically, if a match exists.
	This is the magical function that allows parallelism and locking at the same time. First, the global lock is obtained to search the list for a match. If a match is not found, `nomatch_fun` is called, and then the
	global lock is released and the function returns.
	If a match is found, this function guarantees that a unique lock attached to the match is obtained, and then the global list lock is released. Then, `match_fun` is called with the data
	previously associated to `word`. As a consequence, this function is atomically executed with respect to the specific structure associated to `word`. When that function returns, this unique lock is released, and this
	function returns.
	This means that multiple threads don't necessarily have to wait for each other when they want to do some work on different list nodes. However, if a thread is doing some work on node B, and another thread comes in and
	asks for node B, it will have to wait for the former thread to finish processing, and it will do so while holding the global list lock. That is, if two threads concurrently try to access the same node, one of them will
	have to wait, and will force every other thread trying to access ANY node to wait.
	This is necessary; we can't obtain the unique lock for a node without holding a global lock, otherwise, the unfortunate situation in which we release the global lock; another thread deletes node B, and then we get back and 
	try to lock node B could arise, and we would be in very big trouble.
	User supplied functions (`match_fun` and `nomatch_fun`) are allowed to be `NULL`, in that case, the corresponding pointer to the function is ignored. 
	When `word` was not found in the list, `nomatch_fun` is called with `nomatch_fargs` while holding a global list lock.
	When `word` is found, a unique lock associated to `word` is obtained, the global lock is released, and `match_fun` is called with the generic data that was previously associated to `word` as its first parameter, and
	with `match_fargs` as second parameter.
	This function should only be called with a `match_fun` that can't possibly invoke list operations that will add or delete elements from the list.
	On the other hand, since `nomatch_fun` is called while holding the global lock, it is safe to perform other list operations, as far as it uses operations that do not lock (`list_find_word_nolock()` and others).
	@param list The list to perform the search on.
	@param word A pointer to a null terminated characters sequence holding the word to search for.
	@param match_fun A pointer to a function returning a generic pointer that shall be called if a match is found. Under such scenario, the function is called while holding a unique lock associated to the node found, but 
					 without holding a global lock to the list, to increase parallelism. The first parameter passed to this function is the matching node's data, and the second is `match_fargs`.
					 Because it does not hold a global lock to the list, this function shall not invoke other list operations, especially add or remove.
	@param nomatch_fun A pointer to a function returning a generic pointer that shall be called if a match is not found. Under such scenario, the function is called while holding the global list lock, and it is passed
					   `nomatch_fargs`.
	...
	@param success After this function returns, `*success` will hold `1` if `(*match_fun)()` was called, otherwise it will hold `0`. This allows the caller to have `(*match_fun)()` or `(*nomatch_fun)()` returning `NULL` 
				   and still distinguish between a successfull and failed match.
	@return <ul>
				<li>If no match is found and `nomatch_fun` is `NULL`, then `NULL` is returned.</li>
				<li>If no match is found and `nomatch_fun` is not `NULL`, then the result of evaluating `(*nomatch_fun)(nomatch_fargs)` is returned.</li>
				<li>If a match is found and `match_fun` is `NULL`, then `NULL` is returned.</li>
				<li>If a match is found and `match_fun` is not `NULL`, then the result of evaluating `(*match_fun)(node_data, match_fargs)` is returned.</li>
			</ul>
	@warning The functions must not call `pthread_exit()`.
	@warning Read the documentation carefully, and make sure to understand which locks are active inside `match_fun` and `nomatch_fun`. It is easy to create deadlock situations when not paying attention.
	@note `nomatch_fun` is free to add or delete elements from the list, since it will be executing inside a globally atomic block. Keep in mind that `list_add_nolock()` and `list_delete_nolock()` must be used instead of
		   the regular functions.
		   On the other hand, `match_fund` must not add or delete elements from the list, since `match_fun` is executed without holding a global lock for the list. See `list_find_and_execute_globalock()` for a possible
		   workaround.
*/
...
/** Similar to `list_find_and_execute()`, except that both `match_fun` and `nomatch_fun` are executed while holding a global lock. This must be used everytime `match_fun` can possibly add or delete items from the list.
	First, a global lock for the list is obtained. The list is searched for `word`. 
	When a match is found, it is guaranteed that `match_fun` is called with the data previously associated to `word` and with `match_fargs` only when no other thread is working on the same node. 
	This holds true even for threads that may be working on this node without holding a global lock. It is safe for `match_fun` to commit suicide, i.e., delete the node it's working on, as far as it uses `list_delete_nolock()`.
	No other deletion operations are allowed to be performed by `match_fun` other than deleting the node it's working on. To do so, it must call `list_delete_nolock()`.
	We use a clever little trick to ensure both atomicity on this node and the capability to delete itself: after acquiring the global lock, a unique lock to the node is acquired and then immediately released; 
	by the time it is released,	no other thread can be working on this node, and since the global lock is with us, no one else is stuck trying to lock this specific node. Also, because we released the node's lock, 
	it is safe for the node to destroy itself.
	If a match is not found, `nomatch_fun` is called with `nomatch_fargs` while holding the global list lock.
	@param list The list to perform the search on.
	@param word A pointer to a null terminated characters sequence holding the word to search for.
	@param match_fun A pointer to a function returning a generic pointer that shall be called if a match is found under the scenario described above.
	@param nomatch_fun A pointer to a function returning a generic pointer that shall be called if a match is not found under the scenario described above.
	@param match_fargs This will be passed to `match_fun` as a second parameter when a match is found.
	@param nomatch_fargs This will be passed to `nomatch_fun` when a match is not found.
	@param success After this function returns, `*success` will hold `1` if `(*match_fun)()` was called, otherwise it will hold `0`. This allows the caller to have `(*match_fun)()` or `(*nomatch_fun)()` returning `NULL` 
				   and still distinguish between a successfull and failed match.
	...
	@warning Deletion operations inside `match_fun` are only allowed for itself, i.e., `match_fun` cannot delete a node that is not the same node with which it is currently working. Ignoring this will lead
			 to race conditions, and all bets are off.
*/
...
/** Deletes an entry from a list. If no such entry exists, nothing happens.
	The deletion operation will only take place after both the global lock and a unique lock associated to `word` are obtained. Thus, when an entry is deleted,
	no thread is ever doing some processing with that node.
	@param list The list.
...
/** Deletes an entry from a list. If no such entry exists, nothing happens.
	This function does not try to obtain the global list lock nor the unique lock associated to a node, thus, it should only be called from within a function holding a lock to the list and to the node being deleted.
	@param list The list.
	@param word A null terminated characters sequence denoting the entry to be deleted.
	@return If the word existed, its associated data is returned. Otherwise, `NULL` is returned.
	@warning This function does not free the data associated to a word.
